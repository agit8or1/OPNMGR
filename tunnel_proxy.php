 ...
 *
 * VERSION: 2.0.2 - Fixed HTTPS protocol detection for tunnels
 */
...
// Detect streaming endpoints (Server-Sent Events / EventSource)
$is_streaming = (strpos($path, '/stream') !== false || strpos($path, 'stream_log') !== false);

if ($is_streaming) {
    // For streaming endpoints, we need to stream the response in real-time
    // Disable output buffering and set appropriate headers
    ...
    // Use WRITEFUNCTION to stream data as it arrives
    ...
    curl_exec($ch);
    curl_close($ch);
    exit; // End script immediately for streaming responses
}
...
// This is critical for login flow: POST / → 302 /index.php → 302 /ui/core/dashboard
...
while ($redirect_count < $max_redirects && in_array($http_code, [301, 302, 303, 307, 308])) {
    ...
    curl_close($ch);
    $ch = curl_init("{$protocol}://127.0.0.1:{$tunnel_port}{$path}");
...
if ($response_body === false) {
    ...
    $debug_msg .= "  path: [{$path}]\n";
    $debug_msg .= "  target_url: [{$protocol}://127.0.0.1:{$tunnel_port}{$path}]\n";
    ...
    // Enhanced logging
    log_error("TUNNEL", "Proxy error on session #{$session_id}: {$error}");

    // DON'T immediately close session on single connection error - just log it
    // The tunnel might recover, or it might be a transient network issue
    // Let the cleanup script handle expired/dead sessions
...
if ($is_html || $is_css || $is_js) {
    ...
    if ($is_html) {
        $response_body = preg_replace_callback(
            '/(href|src|action)=["\'](?!http|\/\/|data:|#|\/tunnel_proxy\.php)([^"\']+)["\']/i',
            function($matches) use ($proxy_url) {
                $attr = $matches[1];
                $url = ltrim($matches[2], '/');

                // Skip URLs that are already proxied
                if (strpos($url, 'tunnel_proxy.php') !== false) {
...
<style>
    /* Tunnel proxy styling removed per user request */
</style>
<script>
(function() {
    ...
    function rewriteUrl(url) {
        ...
        // But skip /tunnel_proxy.php itself to prevent double-wrapping
        if (url.startsWith('/')) {
            // Don't rewrite references to tunnel_proxy.php itself
            if (url.startsWith('/tunnel_proxy.php')) {
                console.log('[Tunnel] Skipping tunnel_proxy.php reference:', url);
                return url;
            }
            // Remove leading slash since proxyUrl already ends with path=
            const rewritten = proxyUrl + url.substring(1);
            console.log('[Tunnel] Rewriting absolute:', url, '→', rewritten);
            return rewritten;
        }
        ...
        if (url.match(/^(api|ui)\//)) {
            ...
            console.log('[Tunnel] Rewriting relative API/UI path:', url, '→', rewritten);
            return rewritten;
        }
    ...
    window.URL = new Proxy(OriginalURL, {
        construct: function(target, args) {
            if (args.length > 0 && typeof args[0] === 'string') {
                // Don't rewrite URLs that already contain tunnel_proxy.php
                // This prevents double-wrapping and duplicate path parameters
                if (!args[0].includes('tunnel_proxy.php')) {
                    console.log('[Tunnel] URL constructor rewriting:', args[0]);
                    args[0] = rewriteUrl(args[0]);
                    console.log('[Tunnel] URL constructor rewritten to:', args[0]);
                } else {
                    console.log('[Tunnel] URL constructor skipping tunnel_proxy.php URL:', args[0]);
                }
            }
    ...
    // Helper to fix malformed URLs with duplicate path parameters
    function fixMalformedUrl(url) {
        if (typeof url !== 'string') return url;

        // Check if URL has duplicate path parameters
        // Pattern: tunnel_proxy.php?session=X&path=Y&...&path=Z
        // The regex needs to handle any parameters between the two path= parameters
        if (url.includes('tunnel_proxy.php') && url.includes('&path=')) {
            const parts = url.split('&path=');
            if (parts.length > 2) {
                // Multiple path parameters detected - keep only the first occurrence
                const fixedUrl = parts[0] + '&path=' + parts[1];
                console.log('[Tunnel] Fixed malformed URL with duplicate path:', url, '→', fixedUrl);
                return fixedUrl;
            }
        }
        return url;
    ...
    window.fetch = function(url, options) {
        let processedUrl = rewriteUrl(url);
        processedUrl = fixMalformedUrl(processedUrl);
    ...
    XMLHttpRequest.prototype.open = function(method, url, ...rest) {
        let processedUrl = rewriteUrl(url);
        processedUrl = fixMalformedUrl(processedUrl);
        return originalOpen.call(this, method, processedUrl, ...rest);
    };
