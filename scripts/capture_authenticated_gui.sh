...
if [ -f "/var/www/opnsense/.env" ]; then
    set -a
    source /var/www/opnsense/.env
    set +a
fi

# Configuration - Use environment variables with fallback
USERNAME="${SCREENSHOT_USERNAME:-screenshot}"
PASSWORD="${SCREENSHOT_PASSWORD}"
BASE_URL="${APP_URL:-http://localhost}"
OUTPUT_DIR="${SCREENSHOT_OUTPUT_DIR:-/var/www/opnmanager-website/assets/images}"
COOKIE_JAR="/tmp/opnmanager_cookies.txt"

# Validate required configuration
if [ -z "$PASSWORD" ]; then
    echo "ERROR: SCREENSHOT_PASSWORD not set in environment or .env file"
    echo "Please set SCREENSHOT_PASSWORD before running this script"
    exit 1
fi
