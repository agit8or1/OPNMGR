...
try {
    ...
    $stmt = db()->query("SELECT COUNT(*) FROM firewalls f JOIN firewall_agents fa ON f.id = fa.firewall_id WHERE fa.last_checkin > DATE_SUB(NOW(), INTERVAL 10 MINUTE)");
    $info['active_agents'] = (int)$stmt->fetchColumn();
    
    // Get bug/todo counts
    $stmt = db()->query("SELECT COUNT(*) FROM bugs WHERE status NOT IN ('resolved', 'closed')");
    $info['open_bugs'] = (int)$stmt->fetchColumn();
    
    $stmt = db()->query("SELECT COUNT(*) FROM todos WHERE status NOT IN ('completed', 'cancelled')");
