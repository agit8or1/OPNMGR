...
[![v3.22.1](https://img.shields.io/badge/version-3.22.1-blue)](CHANGELOG.md)
...
| | Version | Notes |
|---|---|---|
| Application | 3.22.1 | Install from `main`. |
...
CI enforces these against `VERSION` and the published artifact — application 3.22.1,
...
release; `main` is at 3.22.1, one documentation patch ahead of it. Tagging has been
intermittent — v3.11.1 was the previous tag, with 3.12 through 3.21 shipped on `main`
and recorded only in the changelog — so [CHANGELOG.md](CHANGELOG.md) remains the
authoritative history rather than the releases page. There is no formal support or LTS
policy, so treat this as actively developed software and read the changelog before
upgrading.
