...
# === LOCKFILE MECHANISM TO PREVENT DUPLICATE AGENTS ===
LOCKFILE="/var/run/opnsense_agent.lock"

# Cleanup function for lockfile
cleanup_lockfile() {
    if [ -f "$LOCKFILE" ]; then
        rm -f "$LOCKFILE"
        log_message "Agent stopped, lockfile removed"
    fi
}

# Set trap to cleanup lockfile on exit/interrupt/termination
trap cleanup_lockfile EXIT INT TERM

# Check for existing agent instance
if [ -f "$LOCKFILE" ]; then
    LOCK_PID=$(cat "$LOCKFILE" 2>/dev/null)
    if [ -n "$LOCK_PID" ] && kill -0 "$LOCK_PID" 2>/dev/null; then
        echo "ERROR: Agent already running (PID: $LOCK_PID)"
        echo "If stale, remove: $LOCKFILE"
        exit 1
    else
        # Stale lockfile - remove it
        rm -f "$LOCKFILE"
        log_message "Removed stale lockfile (old PID: $LOCK_PID)"
    fi
fi

# Create lockfile with current PID
echo $$ > "$LOCKFILE"
log_message "Agent v$AGENT_VERSION started (PID: $$, lockfile: $LOCKFILE)"
...
create_automated_backup() {
    ...
    # Create configuration backup by copying config.xml
    if [ -f "/conf/config.xml" ]; then
        cp /conf/config.xml "$temp_backup" 2>/dev/null
        backup_result=$?
    else
        log_message "ERROR: Cannot create backup - /conf/config.xml not found"
        return 1
    fi
...
execute_command() {
    ...
    case "$command_type" in
        ...
        "shell")
            ...
            timeout 300 sh -c "$command_data" > /tmp/command_output.log 2> /tmp/command_error.log
            cmd_exit_code=$?
            if [ $cmd_exit_code -eq 0 ]; then
                ...
                result="failed_exit_code_${cmd_exit_code}"
            fi
            ;;
        "update_agent")
...
# Report command results with enhanced error logging
report_command_result() {
    local command_id="$1"
    local result="$2"
    local output_file="/tmp/command_output.log"
    local error_file="/tmp/command_error.log"
    
    # Get command output if available
    local output=""
    if [ -f "$output_file" ]; then
        output=$(tail -50 "$output_file" 2>/dev/null | base64 -w 0)
    fi
    
    # Get error output if available
    local error_output=""
    if [ -f "$error_file" ]; then
        error_output=$(tail -20 "$error_file" 2>/dev/null | base64 -w 0)
    fi
    
    # Log locally for troubleshooting
    log_message "Command $command_id result: $result"
    if [ -f "$output_file" ] && [ -s "$output_file" ]; then
        log_message "Command $command_id output: $(tail -5 "$output_file" 2>/dev/null)"
    fi
    if [ -f "$error_file" ] && [ -s "$error_file" ]; then
        log_message "Command $command_id ERROR: $(cat "$error_file" 2>/dev/null)"
    fi
...
" 2>/dev/null)
        ...
        AGENT_CLEANUP=$(echo "$RESPONSE" | python3 -c "
import json, sys
try:
    data = json.load(sys.stdin)
    if data.get('agent_cleanup_requested', False):
        print(data.get('agent_cleanup_command', ''))
except:
    pass
" 2>/dev/null)
        
        # Execute agent cleanup if requested
        if [ -n "$AGENT_CLEANUP" ]; then
            log_message "Agent cleanup requested - executing: $AGENT_CLEANUP"
            log_message "WARNING: This agent will be terminated and replaced"
            
            # Execute cleanup in background and exit this agent
            nohup sh -c "
                sleep 2
                $AGENT_CLEANUP >> /tmp/agent_cleanup.log 2>&1
            " > /dev/null 2>&1 &
            
            log_message "Agent cleanup initiated - exiting current agent"
            exit 0
        fi
        
        # Return the full JSON response so main() can process it
        echo "$RESPONSE"
    else
        log_message "Checkin failed or no response"
        # Return default JSON response
        echo '{"checkin_interval": 300, "pending_requests": []}'
    fi
}
...
# Process pending proxy requests from management server
process_pending_requests() {
    local RESPONSE="$1"
    
    # Check if there are pending requests
    local PENDING_COUNT=$(echo "$RESPONSE" | python3 -c "
import json, sys
try:
    data = json.load(sys.stdin)
    print(data.get('pending_requests_count', 0))
except:
    print(0)
" 2>/dev/null)
    
    if [ "$PENDING_COUNT" -gt 0 ]; then
        log_message "Processing $PENDING_COUNT pending proxy requests"
        
        # Extract and process each request
        echo "$RESPONSE" | python3 << 'EOFPY'
...
try:
    ...
    for req in pending_requests:
        ...
        print(f"Processing request {req_id}: {method} {path}", file=sys.stderr)
        
        try:
            # Make request to local firewall
            ...
            if body and method in ['POST', 'PUT', 'PATCH']:
                req_obj.data = body.encode('utf-8')
            
            # Execute request
            response = urllib.request.urlopen(req_obj, context=ctx, timeout=30)
            response_body = response.read().decode('utf-8', errors='ignore')
            response_code = response.getcode()
            response_headers = dict(response.headers)
            
            # Submit response back to management server
            ...
            submit_req = urllib.request.Request(
                "https://opn.agit8or.net/api/request_queue.php?action=submit_response",
                data=submit_data,
                headers={'Content-Type': 'application/json'}
            )
            urllib.request.urlopen(submit_req, context=ctx, timeout=10)
            print(f"Request {req_id} completed successfully", file=sys.stderr)
            
        except Exception as e:
            print(f"Request {req_id} failed: {str(e)}", file=sys.stderr)
            ...
            try:
                ...
                urllib.request.urlopen(error_req, context=ctx, timeout=10)
            except:
                pass

except Exception as e:
    print(f"Error processing pending requests: {str(e)}", file=sys.stderr)
EOFPY
    fi
}

main() {
    # Perform checkin and get full response
    CHECKIN_RESPONSE=$(perform_checkin)
    
    # Extract interval from response
    ...
    # Process any pending proxy requests
    process_pending_requests "$CHECKIN_RESPONSE"
...
# Main daemon loop
log_message "OPNsense Agent v$AGENT_VERSION starting..."
