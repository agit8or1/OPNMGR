...
$stmt = db()->query("SELECT version, description, release_date FROM platform_versions WHERE status = 'released' ORDER BY created_at DESC LIMIT 1");
$current_version = $stmt->fetch(PDO::FETCH_ASSOC);

// Generate the PDF content using HTML/CSS that browsers can print to PDF
$html_content = generatePageContent($page, $current_version);
...
<?php
function generatePageContent($page, $current_version) {
    switch ($page) {
        case 'about':
            return generateAboutContent($current_version);
        case 'features':
            return generateFeaturesContent();
        case 'changelog':
            return generateChangelogContent();
        case 'documentation':
...
function generateAboutContent($current_version) {
    // Get system stats
    $stmt = db()->query("SELECT COUNT(*) as total_firewalls FROM firewalls");
    $firewall_count = $stmt->fetchColumn() ?: 0;
    
    $stmt = db()->query("SELECT COUNT(*) as open_bugs FROM bugs WHERE status NOT IN ('resolved', 'closed')");
    $open_bugs = $stmt->fetchColumn() ?: 0;
    
    $stmt = db()->query("SELECT COUNT(*) as pending_todos FROM todos WHERE status NOT IN ('completed', 'cancelled')");
