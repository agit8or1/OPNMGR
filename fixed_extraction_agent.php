...
echo "=== FIXING AGENT COMMAND EXTRACTION ==="
echo "Time: `date`"

echo "Creating agent with improved command parsing..."
cat > /usr/local/bin/opnsense_agent.sh << 'EOF'
#!/bin/sh

AGENT_VERSION="5.3_fixed_extraction"
...
log_message() {
    echo "`date '+%Y-%m-%d %H:%M:%S'` [AGENTv5.3] $1" >> "$LOG_FILE"
}
...
JSONEOF
    ...
    if echo "$CHECKIN_RESULT" | grep -q '"success":true'; then
        ...
        # Get commands using improved parsing
        ...
        if echo "$CMD_RESPONSE" | grep -q '"has_command":true'; then
            ...
            # Improved command extraction - get everything between the quotes
...
echo "Testing improved agent..."
/usr/local/bin/opnsense_agent.sh checkin

echo ""
echo "=== IMPROVED AGENT DEPLOYED ==="
echo "✅ Better command extraction with sed"
echo "✅ Full command response logging"
echo "✅ Uses 'sh -c' for proper shell execution"
echo "✅ Agent v5.3_fixed_extraction ready"